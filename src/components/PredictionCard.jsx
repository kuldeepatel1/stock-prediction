...
const PredictionCard = ({
  ...
  const getPriceChange = () => {
    ...
    // Validate both prices are valid positive numbers
    if (typeof currentPrice !== 'number' || isNaN(currentPrice) || currentPrice <= 0) return null;
    if (typeof predictedPrice !== 'number' || isNaN(predictedPrice) || predictedPrice <= 0) return null;

    const change = predictedPrice - currentPrice;
    const changePercent = (change / currentPrice) * 100;

    // If change is essentially zero, return null to hide the section
    if (Math.abs(change) < 0.01 || Math.abs(changePercent) < 0.01) return null;
