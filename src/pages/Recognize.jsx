...
const Recognize = () => {
  ...
  // Default to 1 year from now for meaningful predictions
  ...
  const getChangePercent = (predictedPrice, currentPrice) => {
    if (!currentPrice || currentPrice <= 0 || !predictedPrice || predictedPrice <= 0) return 0;
        ...
        {sorted.length === 0 ? (
          ...
          <div className="divide-y">
            {sorted.map(({ company, prediction }, idx) => {
              ...
              const pct = getChangePercent(prediction.predictedPrice, prediction.currentPrice);
              return (
