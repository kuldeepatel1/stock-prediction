...
const buildSeriesWithProjection = (arr, pred, projectionDaysAfter = 0) => {
...
const formatCurrency = (v) => typeof v === 'number' ? `₹${v.toLocaleString('en-IN', { minimumFractionDigits:2, maximumFractionDigits:2 })}` : '—';
...
const Personalize = () => {
  ...
  const [sellHorizonDays, setSellHorizonDays] = useState(90);
  const [buyWindowDays, setBuyWindowDays] = useState(180);

  const { data: historical } = useQuery({ queryKey: ['historical', ticker], queryFn: () => fetchHistoricalData(ticker), enabled: !!ticker });
  const { data: prediction, isLoading: predLoading } = useQuery({ queryKey: ['prediction', ticker, selectedDate], queryFn: async () => {
            ...
            {(!ticker) ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={series.map(s=>({ date: s.ts, price: s.value }))} margin={{ top: 10, right: 20, left: 10, bottom: 20 }}>
                  ...
                  <Tooltip formatter={(v)=>formatCurrency(Number(v))} labelFormatter={(l)=> new Date(l).toLocaleDateString()} />
